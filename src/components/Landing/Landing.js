  ...
  const slideContent = [
    {
      ...
      description:
        "Discover the future of technology with our cutting-edge solutions and innovative approaches to digital transformation.",
    },
    {
      title: "Innovation at Scale",
      description:
        "Experience seamless integration and powerful performance with our enterprise-grade technology solutions.",
    },
    {
      title: "Transform Your Business",
      description:
        "Leverage our expertise to drive growth and stay ahead in the rapidly evolving digital landscape.",
    },
  ];
    ...
    <div className={`relative w-full h-[98vh] overflow-hidden `}>
        ...
        {images.map((image, index) => (
          ...
          >
            <img
              src={image}
              alt={`Slide ${index + 1}`}
              className="w-full h-full object-cover brightness-[0.3] rounded-b-[50px]"
            />
          </div>
        ))}
      </div>
      ...
      {slideContent.map((content, index) => (
        <div
          ...
          style={{ opacity: index === 0 ? 1 : 0 }}
        >
          <h2 className="text-[60px] md:text-6xl font-medium mb-6">
            {content.title}
          </h2>
          <p className="text-[24px] max-w-xl w-[60%]">{content.description}</p>
          <button className="bg-[#415a77] px-[20px] py-[10px] mt-[30px] rounded-[8px] text-[18px]">
            Learn More
          </button>
        </div>
      ))}
